...
class Billingquotation_crt {
  ...
  Future<List<SelectBillingQuotation>> getSelectBillingQuoatation(
    ...
    final List<Map<String, dynamic>> maps = await dbCustomer.rawQuery(
        " select * from( select cast(a.id as text) id , cast('1' as text) tipo, cast(a.customerId as text) codCustomer, cast(DATE(a.dateQuotation) as text) as fec, cast(a.userId as text) userId, cast(au.strNameUser as text) salesperson, cast(count(c.id) as int) as prods, cast(a.state as int) as numstate, cast(a.total as text) total from quotation a inner join customer b on cast(b.codCustomer as TEXT) = a.customerId inner join QuotationProducts c on a.id = c.quotation_id left join Autentication au on a.userId = au.codUser group by a.id, a.customerId,b.numRucCustomer, a.dateQuotation, a.userId, au.strNameUser, cast(a.state as int) union select cast(a.codBillingUniq as text) id, cast('2' as text) tipo,cast(a.codCustomer as text) codCustomer ,cast(DATE(a.dteBillingDate)as text) fec, cast(a.codUser as text) userId, cast(c.strNameUser as text) salesperson, cast(0 as int) prods,cast(a.flgState as int) as numstate, cast(a.numAmountOperation as text) total from Billing a inner join Customer b on a.codCustomer = b.codCustomer inner join Autentication c on a.codUser = c.codUser) b where 1=1 $tipo_doc $tipo_est order by date(b.fec) desc  ");
