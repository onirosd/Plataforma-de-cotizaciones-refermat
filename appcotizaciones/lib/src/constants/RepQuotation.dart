...
class Reports {
  void reportsEnableds(ReportDataQuotation rquotation) async {
    ...
    } else if (rquotation.company.strPrintFormat == 'formato_simple_igv') {
      reportsFormatoSimpleIGV(rquotation);
    } else if (rquotation.company.strPrintFormat == 'formato_diametros') {
...
//Add rows to grid
    ...
    rquotation.listprodquotationfin.forEach((quotprod) {
      ...
      //     double.parse(quotproduct.theoretical_weight).toStringAsFixed(2);
      row.cells[8].value =
          double.parse(quotproduct.unity_price.toString()).toStringAsFixed(2);
      // row.cells[8].value =
      //     (double.parse(quotproduct.unity_price.toString()) % 1 != 0
      //             ? double.parse(quotproduct.unity_price.toString())
      //                 .toStringAsFixed(2)
...
void reportsFormatoSimpleIGV(ReportDataQuotation rquotation) async {
  // Create a new PDF document.
  //Get external storage directory
  final controller = new MoneyMaskedTextController(
      decimalSeparator: '.', thousandSeparator: ',');
...
//Draws the heading on the page
  ...
  element.brush = PdfBrushes.black;
  result = element.draw(
      page: page, bounds: Rect.fromLTWH(132, result.bounds.bottom + 10, 0, 0))!;

  element = PdfTextElement(text: '$ruc', font: timesRoman);
  element.brush = PdfBrushes.black;
  result = element.draw(
      page: page, bounds: Rect.fromLTWH(132, result.bounds.bottom + 5, 0, 0))!;
  ...
  element = PdfTextElement(
      text: rquotation.customer.numRucCustomer.toString(), font: timesRomanDet);
  element.brush = PdfBrushes.black;
  result = element.draw(
      page: page,
      bounds: Rect.fromLTWH(
  ...
  element = PdfTextElement(text: 'Condicion de Pago', font: timesRomanBoldDet);
  element.brush = PdfBrushes.black;
  result = element.draw(
      page: page,
      bounds: Rect.fromLTWH(
  ...
  element = PdfTextElement(text: rquotation.paycondition, font: timesRomanDet);
  element.brush = PdfBrushes.black;
  result = element.draw(
      page: page,
      bounds: Rect.fromLTWH(
  ...
  element = PdfTextElement(text: rquotation.deliverytype, font: timesRomanDet);
  element.brush = PdfBrushes.black;
  result = element.draw(
      page: page,
      bounds: Rect.fromLTWH(
  ...
  element = PdfTextElement(text: 'Tiempo de Entrega', font: timesRomanBoldDet);
  element.brush = PdfBrushes.black;
  result = element.draw(
      page: page,
      bounds: Rect.fromLTWH(
  ...
  element = PdfTextElement(text: rquotation.deliverytime, font: timesRomanDet);
  element.brush = PdfBrushes.black;
  result = element.draw(
      page: page,
      bounds: Rect.fromLTWH(
...
//Set values to the header cells
  ...
  header.cells[6].value = 'Cantidad';
  // header.cells[8].value = 'Peso Teorico \r\n (KG)';
  header.cells[8].value = 'Precio Unit con IGV';
  header.cells[10].value = 'Precio Final con IGV';
...
//Add rows to grid
  ...
  rquotation.listprodquotationfin.forEach((quotprod) {
    ...
    // row.cells[9].value =
    //     (double.parse(quotproduct.unity_price.toString()) % 1 != 0
    //             ? double.parse(quotproductr.unity_price.toString())
    //                 .toStringAsFixed(2)
    //             : double.parse(quotproduct.unity_price.toString()))
    //         .toString();
    double calculo = double.parse(quotproduct.unity_price.toString()) +
        double.parse(rquotation.company.numImpuesto!) *
            double.parse(quotproduct.unity_price.toString());
    row.cells[8].value = calculo.toStringAsFixed(2);
    row.cells[10].value = (double.parse(calculo.toString()) *
            double.parse(quotproduct.quantity!.toString()))
        .toStringAsFixed(2);
    print(">>>>>> impresion productoooooooooooooooo");
    print(quotproduct);
...
//Draws the grid to the PDF page
  ...
  controller.updateValue(double.parse(rquotation.quotationfin.lgv.toString()));
  String igv = controller.numberValue.toString();
  ...
  gridResult.page.graphics.drawString(
      ...
      subHeadingFont,
      brush: PdfSolidBrush(PdfColor(0, 0, 0)),
      bounds: Rect.fromLTWH(15, gridResult.bounds.bottom + advance, contamos, 0)
      //format: PdfLayoutFormat(layoutType: PdfLayoutType.paginate)
      );
  ...
  //       format: PdfStringFormat(alignment: PdfTextAlignment.left));
  String num_igv =
      (double.parse(rquotation.company.numImpuesto!) * 100).toStringAsFixed(0);
  gridResult.page.graphics.drawString(
      'IGV. $num_igv% :                             $icon_mod$igv',
      PdfStandardFont(PdfFontFamily.helvetica, 14, style: PdfFontStyle.bold),
      brush: PdfSolidBrush(PdfColor(126, 155, 203)),
