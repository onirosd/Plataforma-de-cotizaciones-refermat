...
class CustomerBillingShow extends StatefulWidget {
  CustomerBillingShow({Key? key}) : super(key: key);

  @override
  _CustomerBillingShowState createState() => _CustomerBillingShowState();
}

class _CustomerBillingShowState extends State<CustomerBillingShow> {
  ...
  Widget build(BuildContext context) {
    final billingandflag =
        ModalRoute.of(context)!.settings.arguments as Billingandflag;

    authcrt
        .getDataUserAutentication(billingandflag.billingdata.codUser)
        .then((value) {
      if (this.mounted) {
        setState(() {});
      }
    ...
    custcrt
        .getCustomerBycodUser(billingandflag.billingdata.codCustomer)
        .then((value2) {
      if (this.mounted) {
        setState(() {});
      }
    ...
    return WillPopScope(
      onWillPop: () async {
        ...
        String dir_redirect = "";
        if (billingandflag.switch1 == 1) {
          dir_redirect = 'home';
        } else {
          dir_redirect = 'listBilling';
        }
        Navigator.pushNamedAndRemoveUntil(
            context, dir_redirect, (route) => false,
            arguments: _customer);
            ...
            builder: (BuildContext context,
              ...
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                List<Currency> _out_currency = _currency
                    .where((o) =>
                        o.codCurrency == billingandflag.billingdata.codCurrency)
                    .toList();
                List<BillingType> _out_tipocobro = _tipocobro
                    .where((o) =>
                        o.codBillingType ==
                        billingandflag.billingdata.codBillingType)
                    .toList();
                List<PaymentMethod> _out_metodopago = snapshot.data!
                    .where((o) =>
                        o.codPaymentMethod ==
                        billingandflag.billingdata.codPaymentMethod)
                    .toList();
                List<Bank> _out_banco = _bank
                    .where(
                        (o) => o.codBank == billingandflag.billingdata.codBank)
                    .toList();

                _imprimir.numRecibo =
                    billingandflag.billingdata.codBillingUniq.toString();
                _imprimir.nombreCliente = _customer.strName.toString();
                _imprimir.direccion = _customer.strAddress.toString();
                _imprimir.monto =
                    billingandflag.billingdata.numAmountOperation.toString() +
                        " " +
                ...
                _imprimir.observaciones =
                    billingandflag.billingdata.strComments.toString();
                _imprimir.nroOperacion =
                    billingandflag.billingdata.strOperation;
                _imprimir.vendedor = _auth;
                _imprimir.fecha = billingandflag.billingdata.dteBillingDate;
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  color:
                                      billingandflag.billingdata.flgState == 1
                                          ? Colors.green
                                          : Colors.orange,
                                ),
                                height: 20,
                              ...
                              TextFormField(
                                ...
                                controller: TextEditingController()
                                  ..text =
                                      billingandflag.billingdata.dteBillingDate,
                                decoration:
