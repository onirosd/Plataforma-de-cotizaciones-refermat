...
$bgn            = $decoded_params['dtBgn'];
$end            = $decoded_params['dtEnd'];
...
$sql_quotation = "
    ...
    convert(int, cod_Customer) customerId,
    convert(int, cod_User) userId,
    convert(varchar(100),dte_DateQuotation) dateQuotation,
    ...
    convert(varchar(100),dte_CreateDate) createDate,
    convert(varchar(100),str_CreateUser) createUser,
    convert(varchar(100),dte_UpdateDate) updateDate,
   ...
   convert(varchar(100),qp.dte_CreateDate) create_date,
   ...
   foreach ($conn->query($sql_quotation) as $row) {
    $arr = array(); 
     ...
     if(count($row) > 0){
        ...
        $arr['customerId'] = (int)$row['customerId'];
   ...
   foreach ($conn->query($sql_billing) as $row) {
    $arr2 = array(); 
     ...
     if(count($row) > 0){
        ...
        $arr2['codCustomer'] = (int)$row['codCustomer'];
   ...
   $master_class->quotations                = $arr_deliverytime;
   $master_class->quotations_products       = $arr_indicators;
