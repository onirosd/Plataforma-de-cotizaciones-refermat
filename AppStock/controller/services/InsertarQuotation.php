...
include '../functions/QuotationDB.php';
include '../functions/QuotationProductDB.php';
...
try {
    ...
    $quotationdb = new QuotationDB($conn);
    $quotationproductdb = new QuotationProductDB($conn);

    foreach ($decoded_params as $key => $value) {
        $dato = (array) json_decode($value);
        $num_inserts =  $num_inserts +  $quotationdb->insert((array) $dato['quotat']);
        
        foreach ($dato['listproduct'] as $key => $value) {
            
           // print_r((array)$value);
            $quotationproductdb->insert((array)$value); 
